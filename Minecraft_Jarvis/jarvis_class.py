...
def answerfromwebpage(question, path, model, tokenizer):
    ...
    with open(path, "r", encoding='utf-8') as f:
        ...
        for context in f:
            ...
            save_context = ""
            if len(input_tokens) > 512: #maximum length of context being passed into QA model
                save_context = context #If the total context length is bigger than num, save the current paragraph for the next loop
                ...
                context_string = save_context
            else:
                context_string += context
                #print("CONTEXT: " + context)
                #print("CONTEXT STRING: " + context_string)
                input_tokens = tokenizer.encode(question, context_string)
                if len(input_tokens) > 0: #minimum length before context is passed into QA model
                    cur_answer, cur_start, cur_end = answer_question(question, input_tokens, model, tokenizer)
                    if max_score < (cur_start + cur_end):
                        max_answer = cur_answer
                        max_score = cur_start + cur_end
...
def jarvis(model, tokenizer, query):
